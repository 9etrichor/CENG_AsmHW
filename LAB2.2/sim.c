...
void handle_slli(unsigned int cur_inst) {
    ...
    unsigned int rd = MASK11_7(cur_inst), rs1 = MASK19_15(cur_inst);
    int imm5 = sext(MASK24_20(cur_inst), 12);
...
void handle_srli(unsigned int cur_inst) {
    ...
    NEXT_LATCHES.REGS[rd] = (int)((unsigned)CURRENT_LATCHES.REGS[rs1] >> imm5);
...
void handle_sb(unsigned int cur_inst) {
    ...
    int imm12 = (MASK11_7(cur_inst)) + \
		(MASK31_25(cur_inst) << 5);
    MEMORY[sext(NEXT_LATCHES.REGS[rs1] + imm12, 8)] = sext(CURRENT_LATCHES.REGS[rs2], 8);
}
